...
(function($){
	var loader = {
		show: function($el){
			$el.append('<div class="loader-img flex-center-wrapper"><img class="flex-row" src="assets/images/puff.svg" alt="loader" /></div>');

			return this;
		},
		hide: function($el){
			$el.find('.loader-img').remove();
	...
	var settings = {
		...
		exit_selector: ".slide-exit", // the selector for an exit button in the div if needed, when the exit element is clicked the menu will deactivate, suitable for an exit element inside the nav menu or the side bar
		...
		$.each(track.services, function(i, service){
			$('#music-list').append(
				$('<div>').addClass('music-block')
					...
					.append($('<a>').attr({'href': service.url, 'target': '_blank'})
						.append($('<div>').addClass('music-wrapper')
							.append($('<div>').addClass('img-block').append(
								$('<img>').attr('src', 'assets/logos/' + service.name + '.png')
							))
